...
void FTAG_giveInventory(cClient @client) {
...
void FTAG_playerKilled(cEntity @target, cEntity @attacker, cEntity @inflicter) {
	...
	if((G_PointContents(target.getOrigin()) & CONTENTS_NODROP) == 0) {
		...
		if(ftagAllowPowerupDrop.getBool()) {
...
void FTAG_NewRound(cTeam @loser) {
	...
	for(int i = 0; i < maxClients; i++) {
		...
		for(cFrozenPlayer @frozen = @frozenHead; @frozen != null; @frozen = @frozen.next) {
			if(client.team == frozen.client.team) {
				...
				cVec3 top = mid + cVec3(0, 0, FTAG_DEFROST_RADIUS);
				...
				if(dot(dir, eyemid) >= dot(eyetop, eyemid)) {
					msg += frozen.client.getName() + " (" + ((frozen.defrostTime * 100) / FTAG_DEFROST_TIME) + "%), ";
				}
			}
		}
...
void GT_InitGametype() {
	...
	gametype.setTitle("Freeze Tag");
	gametype.setVersion("0.9");
	gametype.setAuthor("Mike^4JS");
	...
	// add callvotes
	G_RegisterCallvote("ftag_powerups", "1 or 0", "Enables or disables powerups in Freeze Tag.");
	G_RegisterCallvote("ftag_powerup_drop", "1 or 0", "Enables or disables powerup dropping in Freeze Tag.");
